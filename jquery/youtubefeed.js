		...
		*******************************/
			...
			*******************************/
			function loadFeed(){
				...
				$.ajax({
					dataType: "json",
					url: 'https://www.googleapis.com/youtube/v3/search?key=' + settings.key + '&channelId=' + settings.channelId + '&part=snippet,id&order=date&type=video&maxResults=' + settings.count + pageToken,
			...
			*******************************/
			function createHtmlElement(item) {
				...
				var element = settings.pattern
					...
					.replace(/\{\{Date\}\}/g, typeof dateLong != "undefined" ? getOriginDate(dateLong): '')
			...
			*******************************/
				...
				function getOriginDate(date) {
					var momentedDate = moment(date).format('YYYY 年 M 月 D 日');
					return momentedDate
				}
